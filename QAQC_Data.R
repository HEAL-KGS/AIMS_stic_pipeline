...
data_dir <- "YMR_STIC_combined"
fs::dir_ls(data_dir)
stic_files <- list.files(file.path(data_dir), pattern = "\\.csv$")

stic_files <- list.files(path = "YMR_STIC_combined")

for(i in 1:length(stic_files)){
  
  logger_record <- read.csv(file = paste0("YMR_STIC_combined/", stic_files[i]))
  ...
  write_csv(qaqc_stic, file = paste0("YMR_STIC_QAQC/", start_date, "-", end_date,
